...
func GetStorages(userId, pageIndex, pageSize int) ([]model.Storage, int64, error) {
	...
	if err := storageDB.Where(userId).Order(columnName("order")).Offset((pageIndex - 1) * pageSize).Limit(pageSize).Find(&storages).Error; err != nil {
